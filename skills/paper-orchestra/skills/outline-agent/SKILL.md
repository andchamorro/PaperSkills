---
name: outline-agent
description: Generate a structured JSON outline for the PaperOrchestra pipeline. Use when you need to create plotting plans, literature search strategies, and section outlines from idea.md and experimental log inputs. TRIGGER when running Step 1 of paper-orchestra or when "outline generation" is needed for academic paper writing.
---

# Outline Agent

> **Source**: Song et al. (2026), PaperOrchestra, Appendix B - Outline Agent Prompt
...
Divide the field into 2-4 distinct methodology clusters that directly compete with or precede our approach. For each cluster, define:
...
- **Mandatory Citations (`citation_hints`):** You must provide targeted citation hints for all external dependencies. Every hint must point to a single, unambiguous canonical paper.
  ...
  **Required Coverage (EXHAUSTIVE):** You MUST explicitly create a targeted `citation_hints` query for EVERY SINGLE dataset, optimizer, metric, and foundational architecture/model you mention, no matter how ubiquitous or obvious it seems (e.g., AdamW, ResNet, ImageNet, CLIP, Transformer, LLaMA, GPT, LLaVA). If it is in the `experimental_log.md` or `idea.md`, it MUST have a citation hint.
  ...
  Categories requiring citation hints:
...
```json
{
  "plotting_plan": [
    {
      "figure_id": "fig_teaser_cross_modal_alignment",
      "title": "Teaser: Cross-Modal Alignment Performance",
      "plot_type": "plot",
      "data_source": "experimental_log.md",
      "objective": "Visual summary (Radar Chart) demonstrating SOTA balance across 5 metrics.",
      "aspect_ratio": "16:9"
    }
  ],
  ...
  "section_plan": [
    ...
    },
    {
      ...
      "subsections": [
        {
          "subsection_title": "3.1 ...",
          "content_bullets": ["...", "..."],
          "citation_hints": ["Vaswani et al. (Attention Is All You Need)", "..."]
        }
      ]
    }
  ]
...
After generating the outline, the orchestrator will validate:
1. JSON is syntactically valid
2. All three top-level keys are present
3. `plotting_plan` entries have required fields
4. `section_plan` has exhaustive `citation_hints` coverage
