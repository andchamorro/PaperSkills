...
async def batch_fetch(
    ...
    tasks = [asyncio.create_task(fetch_paper_metadata(paper, semaphore)) for paper in papers]
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch fetch paper metadata with concurrency control"
    )
    parser.add_argument("--input", required=True, help="Path to input JSON file")
    parser.add_argument(
        "--max-concurrent", type=int, default=10, help="Max concurrent API requests"
    )
