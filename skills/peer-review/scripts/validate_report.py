...
def validate_report(html: str, evaluator_json: dict | None = None) -> dict:
    ...
    checks.append({
        ...
        "passed": all_criteria,
        "issue": None if all_criteria else f"Missing criteria: {set(criteria) - set(found_criteria)}",
    })
    ...
    if radar_match:
        try:
            ...
            checks.append({
                ...
                "issue": None if (has_8_values and in_range) else
                    f"Found {len(values)} values, range check: {in_range}",
            })
        except ValueError:
            checks.append({
                "check": "Radar chart data (8 values, range 1-5)",
    ...
    rec_keywords = ["ACCEPT", "MINOR REVISIONS", "MAJOR REVISIONS", "REJECT",
                     "接受", "小修", "大修", "拒绝"]
...
def main():
    ...
    if args.evaluator:
        try:
            with open(args.evaluator, "r", encoding="utf-8") as f:
